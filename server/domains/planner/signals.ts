...
export function computeMarksShare(units: RawUnit[], pyqMarksByUnit: Record<string, number>): Record<string, number> {
  ...
  if (totalPyq >= cfg.minPyqMarksToTrust) {
    ...
    for (const u of units) {
      ...
      raw[u.id] = (hit ? hit[1] : cfg.fallbackDefault) / (perSubject[u.subjectId] ?? 1);
    }
  }
  const max = Math.max(0, ...Object.values(raw));
  const out: Record<string, number> = {};
  for (const u of units) out[u.id] = max > 0 ? (raw[u.id] ?? 0) / max : 0;
  return out;
}
...
export function buildUnitSignals(raw: RawInput, now: Date): UnitSignals[] {
  ...
  return raw.units.map((u, idx) => {
    ...
    const weak = touched
      .filter((s) => s.mastery != null && s.mastery < weakBelow)
    ...
    return {
      ...
      mastery: mean(touched.map((s) => s.mastery).filter((m): m is number => m != null)),
      attempts,
      correct,
