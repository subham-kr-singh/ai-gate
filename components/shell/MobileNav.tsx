...
export function MobileNav({ active = "today", initial = "G" }: { active?: NavKey; initial?: string }) {
  ...
  return (
    <div className="lg:hidden">
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-line bg-surface/95 px-4 py-2 backdrop-blur">
        <Link href="/dashboard" className="flex items-center gap-2 text-sm font-semibold text-ink">
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-ink text-xs font-semibold text-white" aria-hidden="true">
            G
          </span>
          GATE AI
        </Link>
          ...
          >
            {NAV_GROUPS.map((group) => (
              <section key={group.label}>
                <p className="mb-2 text-xs font-medium uppercase tracking-wide text-slate">{group.label}</p>
                      ...
                      <li key={key}>
                        <Link
                          ...
                          className={`flex items-center gap-3 rounded-2xl px-3 py-2 text-sm ${
                            isActive ? "bg-ink text-white" : "text-ink hover:bg-control"
                          }`}
                        >
                          {item.icon}
                          {item.label}
            ...
            <div className="mt-auto flex items-center gap-3 border-t border-line pt-5">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-lavender text-xs font-semibold" aria-hidden="true">
                {initial}
              </span>
              <span className="text-sm text-slate">Signed in</span>
            </div>
          </nav>
        </div>
      )}
      ...
      <nav
        aria-label="Main"
        className="fixed inset-x-0 bottom-0 z-30 flex items-stretch justify-between gap-1 border-t border-line bg-surface/95 px-2 py-1 backdrop-blur"
      ...
      >
        {MOBILE_NAV_ITEMS.map((n) => {
          ...
          return (
            ...
            >
              <span className={`flex h-8 w-8 items-center justify-center rounded-full ${isActive ? "bg-ink text-white" : ""}`}>
                {n.icon}
              </span>
