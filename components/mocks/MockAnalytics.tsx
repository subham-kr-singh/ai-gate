  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Tile tone="butter" label="Score" value={`${marks(a.score.obtained)} / ${a.score.max}`}
        ...
        <Tile tone="sky" label="Accuracy" value={pct(a.accuracy)}
          note={`${a.counts.correct} correct · ${a.counts.wrong} wrong`} />
        <Tile tone="lavender" label="Attempt rate" value={`${a.counts.attempted} / ${a.counts.total}`}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <Card title="Did changing answers help?">
          {changes.changedCount === 0 ? (
            <p className="text-sm text-slate">You did not change any answers.</p>
          ) : (
            <>
        ...
        <Card title="Guessing">
          {a.guessing.guessedAttempted === 0 ? (
            <p className="text-sm text-slate">You did not flag any answers as guesses.</p>
          ) : (
            <>
        ...
        <Card title="Marked for review">
          {a.reviewed.marked === 0 ? (
            <p className="text-sm text-slate">You did not mark any questions.</p>
          ) : (
            <>
      ...
      {weakUnits.length > 0 && (
        <section className="rounded-[20px] border border-line p-5">
          <h3 className="mb-3 font-semibold text-ink">Needs attention</h3>
          <ul className="flex flex-col gap-2 text-sm">
            {weakUnits.map((r) => (
              <li key={r.key} className="flex items-center justify-between gap-4">
                <span className="text-ink">{r.label}</span>
                <span className={judge(r, a.score.share) === 'weak' ? 'text-amber' : 'text-slate'}>
...
function Tile(props: { tone: PastelTone; label: string; value: string; note: string; delta?: { text: string; good: boolean } | null }) {
  return (
    <div className={`rounded-[20px] p-5 ${PASTEL_CLASS[props.tone]}`}>
      <p className="text-xs text-body-muted">{props.label}</p>
      <div className="mt-1 flex items-baseline gap-2">
        <p className="text-2xl font-semibold text-ink">{props.value}</p>
        {props.delta && (
          <span className={`text-xs font-medium ${props.delta.good ? 'text-teal' : 'text-amber'}`}>{props.delta.text}</span>
        )}
      </div>
      <p className="mt-1 text-xs text-body-muted">{props.note}</p>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-4 text-sm">
        <dt className="text-body-muted">{k}</dt>
        <dd className={`font-medium ${tone === 'good' ? 'text-teal' : tone === 'weak' ? 'text-amber' : 'text-ink'}`}>{v}</dd>
      </div>
            ...
            <li key={r.key} className="py-3">
              <div className="flex items-baseline justify-between gap-4 text-sm">
                ...
                <span className="shrink-0 text-ink">
                  ...
                  <span
                    className={`ml-2 text-xs ${j === 'weak' ? 'text-amber' : j === 'good' ? 'text-teal' : 'text-slate'}`}
                  >
              ...
              <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-black/10" aria-hidden>
                <div className="animate-fill-w h-full rounded-full bg-teal" style={{ width: `${Math.max(0, Math.min(1, r.scoreShare)) * 100}%` }} />
              </div>
