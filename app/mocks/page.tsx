    ...
    <AppShell active="mocks">
      <div className="flex flex-col gap-7">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h1 className="text-xl font-semibold text-ink">Mocks</h1>
        ...
        {active && (
          <section className="rounded-[24px] bg-teal p-7 text-white md:p-8">
            ...
            <Link href={`/mocks/${active.id}`} className="mt-5 inline-flex h-10 items-center rounded-full bg-white px-5 text-sm font-medium text-ink">
              Resume
            </Link>
          </section>
        )}
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,1fr)_320px]">
          <div className="rounded-[20px] border border-line p-5">
            <h2 className="font-semibold text-ink">{bp.title}</h2>
            <p className="mt-1 text-sm text-slate">
                ...
                {listing.ready.map((m) => (
                  <li key={m.id} className="flex items-center justify-between py-3">
                    <span className="text-ink">
                      {m.title} <span className="text-slate">· built {shortDate(m.createdAtMs)} · {m.freshCount} new questions</span>
                    </span>
                    <Link href={`/mocks/${m.id}`} className="font-medium text-ink underline underline-offset-2">
                      Open
                    </Link>
                  </li>
                ))}
          ...
          <div className="rounded-[20px] border border-line p-5">
            ...
            {active && <p className="mt-3 text-xs text-slate">Finish the running mock before starting another.</p>}
          </div>
        </section>
        ...
        <section>
          <div className="mb-3 flex items-center justify-between">
            ...
            {r.averageScoreShare !== null && (
              <span className="text-sm text-slate">Average score {pct(r.averageScoreShare)}</span>
            )}
          </div>
          {listing.history.length === 0 ? (
            <p className="rounded-[20px] border border-line p-5 text-sm text-slate">
  ...
  return (
    <li className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm">
      <div>
        ...
        <p className="text-xs text-slate">{m.submittedAtMs ? shortDate(m.submittedAtMs) : ''}</p>
      </div>
      ...
      <Link href={`/mocks/${m.id}/result`} className="font-medium text-ink underline underline-offset-2">
        View result
      </Link>
    </li>
  );
