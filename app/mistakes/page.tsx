...
export default async function MistakesPage(props: { searchParams: Promise<SP> | SP }) {
  ...
  return (
    <AppShell active="mistakes" initial={initial}>
      <div>
        <h1 className="text-xl font-semibold text-[#111111]">Mistake notebook</h1>
      ...
      <section aria-label="Open mistakes by type" className={`${ui.card} p-5`}>
        <p className="text-sm font-semibold tabular-nums text-[#111111]">
          {summary.open} open, <span className={summary.untagged > 0 ? "text-[#D98E2B]" : ""}>{summary.untagged} not tagged yet</span>
        </p>
        <ul className="m-0 mt-2 flex list-none flex-wrap gap-x-5 gap-y-1 p-0 text-sm tabular-nums">
          {MISTAKE_TYPES.map((t) => (
            <li key={t}><span className="text-[#77736D]">{MISTAKE_TYPE_LABELS[t]}</span> {summary.byType[t]}</li>
          ))}
        </ul>
      </section>
      ...
      <form method="get" className="flex flex-wrap items-end gap-3">
        ...
        {filtered && <a href="/mistakes" className="pb-2 text-sm text-[#77736D] underline">Clear filters</a>}
      </form>
          ...
          {items.map((m) => (
            <li key={m.id} className="p-5">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                ...
                <time dateTime={m.createdAt.toISOString()} className="text-xs tabular-nums text-[#77736D]">
                  {m.createdAt.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
                </time>
              </div>
              {m.unitLabel && (
                <p className="text-xs text-[#77736D]">{m.subjectLabel ? `${m.subjectLabel}, ` : ""}{m.unitLabel}</p>
              )}
              <p className="mt-1 text-sm">
                <span className={m.mistakeType ? "text-[#111111]" : "text-[#D98E2B]"}>
                  {m.mistakeType ? MISTAKE_TYPE_LABELS[m.mistakeType] : "Not tagged yet"}
                </span>
                {m.resolved && <span className="text-[#0E8074]">, resolved</span>}
              </p>
              {m.note && <p className="mt-1 max-w-[70ch] text-sm text-[#3a3a3a]">{m.note}</p>}
