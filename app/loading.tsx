  ...
  return (
    <AppShell active="today">
      <div className="flex flex-col gap-7" aria-busy="true" aria-label="Loading">
        <div className="h-6 w-44 animate-pulse rounded-full bg-[#ECE9E3]" />
        <div className="h-32 animate-pulse rounded-[24px] bg-[#ECE9E3]" />
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {[0, 1, 2].map((i) => (
            <div key={i} className="h-24 animate-pulse rounded-[20px] bg-[#ECE9E3]" />
          ))}
        </div>
      </div>
    </AppShell>
