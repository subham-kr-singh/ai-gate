...
export function scheduleNextReview(
  ...
  if (!correct) {
    ...
    return { stage: 0, lapses: (prev?.lapses ?? 0) + lapse, dueAt: due(ladder[0]), lastReviewedAt: now };
  }
  if (!prev) {
    const stage = Math.min(1, last);
    return { stage, lapses: 0, dueAt: due(ladder[stage]), lastReviewedAt: now };
  }
  ...
  return { stage, lapses: prev.lapses, dueAt: due(ladder[stage]), lastReviewedAt: now };
}
...
export function initialReviewAfterStudy(now: Date, ladder: readonly number[] = DEFAULT_LADDER_DAYS): ReviewSnapshot {
  return { stage: 0, lapses: 0, dueAt: new Date(now.getTime() + ladder[0] * DAY_MS), lastReviewedAt: now };
}
